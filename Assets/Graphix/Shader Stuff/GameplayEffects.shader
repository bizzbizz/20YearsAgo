...
Shader "20YA/GameplayEffects"
{
	Properties
	{
		...
		//_NScale("Noise scale", Vector) = (1., 1., 1.)
		//_NColor("Noise Color", Vector) = (0.2, 0.2, 0.2)
...
/*				inline float4 getVideo(float2 uv)
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					float4 col = tex2D(_MainTex,
						...
						_IsHazy * 
						(
							uv + (1 + _TimeX * .1) * .002 * float2(sin(uv.y * m) , cos(uv.x * m))
						)
					);

					float3 color = 
						(1.0 - _IsGhost) * (0.5 + fl) * col.xyz
						+ _IsGhost * video;
					float alpha = 
						(1.0 - _IsGhost) 
